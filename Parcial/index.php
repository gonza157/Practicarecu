    ...
    switch($path)
    {
        case"/usuario":
            {                
                ...
                //$U1->T_usuario = $_POST["Tipodeuser"]; 
                if(!empty($foto))
                {
                    $extencion = explode('.',$_FILES["foto"]["name"]);                
                    // Archivos::MoverArchivo($_FILES["Foto"]["tmp_name"],"./imagenes/".$U1->email.".".$extencion[1]);
                    Archivos::GuardarImagen();
                    $nombre = '--'.time().'--.'.$extencion[1];
                    $U1->foto = "./imagenes/".$nombre;//"./imagenes/".$U1->email.".".$extencion[1];
